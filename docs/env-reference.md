...
## Config Files By Workflow

- `config/local.compose.env`: compose-driven local stack commands such as
  `make local-up`, `make local-down`, and `make local-logs`.
- `config/local.api.env`: host-run backend commands `make db-migrate` and
  `make api-dev`.
- `config/staging.compose.env`: staging compose simulation via
  `make staging-up`, `make staging-down`, and `make staging-smoke`.
- `config/prod.compose.env`: image build/publish/release commands
  `make deploy-build`, `make deploy-publish`, and `make deploy-release`.
- `config/vps.compose.env`: generated by `make vps-install` and consumed by the
  single-VPS commands such as `make vps-up`, `make vps-down`, `make vps-smoke`,
  and `make vps-upstall`.

## Frontend Env Behavior

The frontend `NEXT_PUBLIC_*` variables are not loaded the same way in every
workflow:

- `local`: the Next.js dev container reads them when the web container starts.
  After editing `config/local.compose.env`, rerun `make local-up`.
- `staging`, `vps`, and built `prod` images: the web image bakes these values
  in at build time. After editing the corresponding `*.compose.env` file,
  rerun the relevant build/deploy command before expecting browser behavior to
  change.
